  ...
  var BlobBuffer = function (fs) {
    return function (destination) {
      ...
      this.write = function (data) {
        ...
        var isAppend = newEntry.offset >= this.length
          ...
          if (fd) {
            return new Promise(function (resolve, reject) {
              convertToUint8Array(newEntry.data).then(function (dataArray) {
                ...
                var buffer = Buffer.from(dataArray.buffer)

                var handleWriteComplete = function (err, written, buffer) {
...
(function () {
  var WebMWriter = function (ArrayBufferDataStream, BlobBuffer) {
    ...
    return function (options) {
      ...
      var DEFAULT_TRACK_NUMBER = 1

      var writtenHeader = false

      var videoWidth; var videoHeight

      var clusterFrameBuffer = []

      var clusterStartTime = 0
      ...
      var blobBuffer = new BlobBuffer(options.fileWriter || options.fd)
      ...
      function writeCues () {
        ...
        var cuesBuffer = new ArrayBufferDataStream(16 + cues.length * 32) // Pretty crude estimate of the buffer size we'll need
      ...
      function flushClusterFrameBuffer () {
        ...
        var
          buffer = new ArrayBufferDataStream(rawImageSize + clusterFrameBuffer.length * 32)
