...
void main() {
  ...
  vec3 r1 = rect(st, vec2(.04, .05), vec2(.35, .7), red1);

  vec3 r2 = rect(st, vec2(0.35, 0.25), vec2(0.35, 0.7), red2);
  vec3 r3 = rect(st, vec2(0.75, 0.25), vec2(0.25, 0.7), orange);


  // color += r1 + r2 + r3;
  color += mix(r1,r2,1. - st.x) + mix(r2,r3, 1.- st.x) / mix(r1/r2,r3*r2/r1,1.0-st.y);
